...
async function scrapeOpenGovPortal(portal) {
  try {
    for (const url of openGovFetchUrls(portal)) {
      ...
      // If a ScraperAPI key is in Render, route the request through their residential proxies to bypass Cloudflare
      if (process.env.SCRAPER_API_KEY) {
        targetUrl = `http://api.scraperapi.com?api_key=${process.env.SCRAPER_API_KEY}&url=${encodeURIComponent(url)}&render=true`;
        fetchHeaders = {}; // The proxy API handles all browser headers automatically
        console.log(`[OpenGov] Routing through Proxy API...`);
      }
